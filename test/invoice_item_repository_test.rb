...
class InvoiceItemTest < Minitest::Test
  ...
  def test_find_by_id
    invoice_items = @invoice_item_repo.find_by_id(1)
    invoice_items1 = @invoice_item_repo.find_by_id(45)
    assert_equal 1, invoice_items.size
    assert_equal 0, invoice_items1.size
  end

  def test_find_by_item_id
    invoice_items = @invoice_item_repo.find_by_item_id(539)
    assert_equal 1, invoice_items.size
  end

  def test_find_by_invoice_id
    invoice_items = @invoice_item_repo.find_by_invoice_id(1)
    assert_equal 3, invoice_items.size
  end

  def test_find_by_quantity
    invoice_items = @invoice_item_repo.find_by_quantity(1)
    invoice_items1 = @invoice_item_repo.find_by_quantity(5)
    assert_equal 0, invoice_items.size
    assert_equal 1, invoice_items1.size
  end

  def test_find_by_created_at
    invoice_items = @invoice_item_repo.find_by_created_at("2012-03-27 14:54:09 UTC")
    invoice_items1 = @invoice_item_repo.find_by_created_at("2016-01-01")
    assert_equal 3, invoice_items.size
    assert_equal 0, invoice_items1.size
  end

  def test_find_by_updated_at
    invoice_items = @invoice_item_repo.find_by_updated_at("2012-03-27 14:54:09 UTC")
    invoice_items1 = @invoice_item_repo.find_by_updated_at("2016-01-01")
    assert_equal 3, invoice_items.size
    assert_equal 0, invoice_items1.size
  end
end
